...
void ofApp::update(){
	...
	if(cam.isFrameNew()) {
        ...
        //Getting a new frame
        clrImage.setFromPixels( cam.getPixels() );
		...
		//Smoothing image
		blurred = grayImage;
		blurred.blurGaussian( (scaledVol*50) );

        //#### FIND INTERESTING POINTS #####
		Mat imageCV;
		imageCV = cvarrToMat( blurred.getCvImage() );
		goodFeaturesToTrack(imageCV, corners, 1000, 0.0001, 1); //param 3 is how many points you want to get
    }
...
void ofApp::draw(){
    ...
    ofPushStyle();
        ofPushMatrix();
            ...
            float w = clrImage.width;
            float h = clrImage.height;

            float scaleRatio = winW/w;

            ofScale(-scaleRatio, scaleRatio);
            ofTranslate(-w, 0);

            //Original decimated image
            //clrImage.draw(0, 0, w, h);

            //blurred immage
            blurred.draw(0, 0, w, h);

            ofSetColor(255, 0, 0);
            ofFill();
            for (int i=0; i<corners.size(); i++) {
                ofDrawEllipse(corners[i].x, corners[i].y, 0.5, 0.5);
            }
